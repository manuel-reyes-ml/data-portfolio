...
├── .opencode/         # OpenCode side of the dual harness — agents/ (docs-fix, docs-sync, eval-guardian,
...
│                      # learn, pattern-scout, security-auditor), commands/ (commit-msg, draft-issue,
│                      # eval, labels, pr-prep, readme, review, task-brief, test),
...
### 🎓 Take-order — the 1099 / DataVault DE-AE flagship

| # | Course / Certification | Source | Cost | Stage | Why here, in this position |
|---|---|---|---|---|---|
| 1 | Python for Everybody Specialization | Coursera · U. Michigan | Free (audit) | S1 | Language floor before anything is written for a regulated pipeline. |
| 2 | Mode Analytics SQL Tutorial | Mode | Free | S1 | SQL fluency — the ingest/reconcile logic is expressed in SQL long before dbt. |
| 3 | CS50P — Introduction to Programming with Python | HarvardX | Free | S1 | Unit tests and debugging; the reconciliation gate is only trustworthy if it is tested. |
| 4 | uv — Python Packaging & Environments | Astral docs + Sweigart quickstart | Free | S1 | **Take before the first commit** — every repo standard downstream assumes `uv.lock` exists. |
| 5 | Pre-Commit Hooks — Molin four-part series | Blog series | Free | S1 | Immediately after uv: hooks must exist before there is history to protect. |
| 6 | Introduction to Git and GitHub | Coursera · Google | Free (audit) | S1 | Branch → PR → self-review. Solo-repo fluency is proven; team workflow is the untested skill. |
| 7 | Architecture Documentation: C4 + ADR | c4model.com + AWS Prescriptive Guidance | Free | S1 | Before the first architecture decision, so ADR-001 is written properly rather than back-filled. |
| 8 | AI Python for Beginners | DeepLearning.AI | Free | S1 | LLM basics ahead of the S1 exception-explanation layer. |
| 9 | Building with the Claude API | Anthropic Academy | Free | S1 | Structured outputs — the exception explainer must return typed objects, not prose. |
| 10 | Pre-processing Unstructured Data for LLM Apps | DeepLearning.AI | Free | S1 | Exception-document parsing. |
| 11 | Docker for Beginners with Hands-on Labs | KodeKloud | Free | S1 | Containerize before deploy work begins. |
| 12 | 30 Days of Streamlit | Streamlit | Free | S1 | Thin review surface for reconciliation exceptions. |
| 13 | MITx 6.00.1x — CS & Programming with Python | edX · MIT | Free (audit) | S1 | CS foundations; runs in the background, no dependency blocks on it. |
| 14 | 🎖️ **AI-901** Azure AI Fundamentals | Microsoft · Pearson VUE | **$99** ✅ | S1 | Cheapest committed exam; take once S1 build work is underway. |
| 15 | ⏸️ **AB-620** AI Agent Builder Associate | Microsoft | ~$165 — **CONDITIONAL** | S1–S2 | **Do not take by default.** Single trigger: a deliberate Microsoft-ecosystem specialization. |
| 16 | PostgreSQL for Everybody + use-the-index-luke.com | Coursera · U. Michigan + web | Free (audit) | S2 | Opens S2 — warehouse modelling and query plans before dbt is introduced. |
| 17 | dbt Fundamentals | dbt Labs | Free | S2 | The core AE skill; the reconciliation rules move into tested models here. |
| 18 | dbt Advanced Learning Paths (Analytics Engineering) | dbt Labs | Free | S2 | Straight after Fundamentals — this is the Analytics-Engineer differentiator. |
| 19 | ⚡ Dataframe Engine Boundary — Polars-first pipelines | Polars User Guide (roadmap S2 row 6.5) | Free | S2 | **The course behind the S2 optional engine upgrade.** Take before touching ingestion. |
| 20 | Astronomer Academy — Airflow 101 + DAG Authoring | Astronomer | Free | S2 | Orchestration once there are tested models worth scheduling. |
| 21 | Apache Kafka 101 | Confluent Developer | Free | S2 | Streaming literacy for the ingestion story. |
| 22 | Terraform Fundamentals | HashiCorp Developer | Free | S2 | Infrastructure as code ahead of the ECS deploy. |
| 23 | 🆕 IBM AI-Native Data Engineering PC | Coursera · IBM (CORRECTION 43) | Coursera Plus | S2 | All three gap-filling courses land here: PII-safe corpora, vector schemas + retrieval governance for the S3 analyst layer, reproducible ML-ready datasets. |
| 24 | Snowflake Data Engineering PC | Coursera · Snowflake | Coursera Plus | S2 | The one warehouse taken deep, paired with dbt. |
| 25 | Databricks Academy — Data Engineering with Spark | Databricks | Free | S2 | Spark literacy; lakehouse breadth. |
| 26 | BigQuery for Data Analysts | Google Cloud | Free | S2 | Awareness only — enough to speak to a GCP stack. |
| 27 | 🎖️ **DP-700** Fabric Data Engineer | Microsoft | **$165** ✅ | S2 | After the dbt + Airflow work exists to talk about. |
| 28 | 🎖️ **AWS DEA-C01** Data Engineer Associate | AWS | **$150** ✅ | S2 | Immediately after DP-700; the S3/Glue/Kinesis surface is the deploy target. |
| 29 | ⏸️ Lakehouse slot — **take exactly ONE**: DP-750 / SnowPro Core / Databricks DE | Microsoft / Snowflake / Databricks | $165–200 — **CONDITIONAL** | S2 | Deferred on purpose — the deciding input is the target employer's stack. |
| 30 | ⏸️ **dbt Analytics Engineering** | dbt Labs | ~$200 — **CONDITIONAL** | S2 | Only if the apply-list names it; the tested dbt project is the real signal. |
| 31 | AI Agents in LangGraph | DeepLearning.AI | Free | S3 | Opens S3 — the HITL pattern behind gated AI corrections. |
| 32 | LangChain Academy (LangGraph + LangSmith) | LangChain | Free | S3 | Tracing and evaluation for the analyst layer. |
| 33 | Automated Testing for LLMOps | DeepLearning.AI | Free | S3 | Regression gates on the NL-query surface. |
| 34 | MCP — Advanced Topics (full) | Anthropic Academy | Free | S3 | Text-to-SQL / RAG-over-marts exposed as tools. |
| 35 | Improving Accuracy of LLM Applications | DeepLearning.AI | Free | S1 | Revisited at depth once there is a real eval set. |
| 36 | 🎖️ **Anthropic CCA-F** | Anthropic · Pearson VUE | **~$125** ✅ ⚠️ | S3 | Agentic orchestration source-of-truth. |
| 37 | 🎖️ **AI-103** Azure AI Apps & Agents Developer | Microsoft | **$165** ✅ | S3 | Code-first Azure/Foundry — the committed Microsoft credential. |
| 38 | 🎖️ **Databricks GenAI Engineer Associate** | Databricks | **$200** ✅ | S3 | Optional; reads as both a GenAI and a DE cert. |
