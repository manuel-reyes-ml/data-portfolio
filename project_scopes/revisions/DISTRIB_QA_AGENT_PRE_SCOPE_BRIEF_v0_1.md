...
| KMC distribution forms (`.pdf`) | ✅ Read to derive field map | 🚫 Never — filenames alone name plan sponsors | Client identifiers |
...
| **B4** | Field labels only from one KMC form (**labels, no values, no plan name**) | Same — defines the field map | Paste as text |
