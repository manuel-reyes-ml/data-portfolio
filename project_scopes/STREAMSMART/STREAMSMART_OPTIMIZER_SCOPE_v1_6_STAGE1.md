# 📺 STREAMSMART OPTIMIZER — Stage 1 Project Scope v1.6

## AI-Powered Streaming Subscription Rotation Advisor
## "Spend Less, Watch More" — Intelligent Subscription Optimization Dashboard

**Document Version:** 1.6 (🎯 **v10.0 ALIGNMENT** — Stage-1 foundation; Supporting/backlog; 3-stage model; destination Applied AI Engineer → FDE. Prior v1.5 note archived below.)
**Last Updated:** August 10, 2026  
**Status:** 📋 DRAFT — Awaiting Approval  
**Author:** Manuel Reyes  
**Stage:** 1 (Foundation) — evolves S1 → S2 → S3 within the v10.0 3-stage model.
...
**Portfolio role:** 🧩 **Supporting — backlog** (production-grade) — Stage 1 of StreamSmart; consumer/personal-finance domain. In v10.0, **flagship vs supporting = size & emphasis, not a quality tier — every project is production-grade.** Lead projects get new tooling first and are updated continuously as skills grow.

**Stage-evolution arc:**

| Stage | Theme | This project's layer |
|---|---|---|
| **S1** | Foundation (GenAI-first core) | Advisory dashboard — content/pricing-API ingestion + subscription-optimization advisor. |
...
| **S3** | Applied AI (RAG/agentic + eval) | Agentic subscription manager — autonomous rotation with HITL + eval + Phoenix. |
...
## 15. Project Evolution (3 Stages)

This project evolves through the roadmap's **3 stages (v10.0)** — one system, never rebuilt per stage:

| Stage | Role (v10.0) | Enhancements |
|-------|------|--------------|
| **S1 (THIS SCOPE)** | Foundation (GenAI-first core) | Advisory dashboard + AI rotation planner + content search |
| **S2** | DE/AE hardening | AWS deployment, PostgreSQL, **dbt models + contracts**, Airflow price-monitoring pipeline, Kafka ingestion, vector DB for content similarity |
| **S3** | Applied AI (agentic + eval) | Agentic rotation via **LangGraph orchestrator-workers** (cancel / resubscribe / content-scout) with **mandatory human checkpoint before any action**; **MCP** + Playwright browser automation; LLM-as-judge eval gate + Phoenix tracing. ML prediction (churn / collaborative-filtering) folds in here as an **earned-overlay**. **Never stores credentials, never enters payment.** *Optional beyond-portfolio: multi-tenant SaaS, monetization, mobile.* |

### Stage 1 → S3 Evolution (3-stage model)
...
STAGE 1 (NOW):          "Tell me what to do" (Advisory AI)
  ...
  │
  ▼
STAGE 3 (FUTURE):       "Do it for me" — agentic, with mandatory human approval
```
...
- [ ] Scope appropriately focused on Stage 1 (S2/S3 layers deferred, not built now)
...
> 🔒 **Personal-finance rule:** no personal account data, balances, or real subscription records in the public repo — synthetic fixtures only.

| # | Heading | What goes under it | What does *not* |
|---|---------|--------------------|-----------------|
| **①** | **Production** | Deploy path, eval/test gates, and — if it has real users — retention and failure behaviour. Backlog-tier: honesty about deployment status beats an inflated Production section. | A stack list is not a production claim. If nothing depends on it and nothing watches it, it is not in production — say so and move the content to Architecture. |
| **②** | **Cost** | Runtime and inference cost per run, and the *user-facing* saving the optimiser identifies stated as a **methodology**, not as a personal figure. | A number with no mechanism. And never a speed/cost win without its reliability disclosure — state the SLA the change held to. A win that hides a regression is the bait-and-switch reviewers watch for. |
| **③** | **Architecture** | ADR set + C4 Context; the ingestion → normalise → detect → recommend boundary. | Diagrams shown without the decision behind them. The ADR is what turns a diagram into evidence of judgement. |
...
### 🎓 Take-order — StreamSmart (consumer AI, backlog)
...
| 4 | Building with the Claude API | Anthropic Academy | Free | S1 | Structured outputs for the recommendation surface. |
| 5 | 30 Days of Streamlit | Streamlit | Free | S1 | The app surface. |
| 6 | Docker for Beginners with Hands-on Labs | KodeKloud | Free | S1 | Containerize. |
| 7 | 🎖️ **AI-901** Azure AI Fundamentals | Microsoft · Pearson VUE | **$99** ✅ | S1 | Shared with the lead projects; nothing StreamSmart-specific. |
| 8 | ⏸️ **AB-620** AI Agent Builder Associate | Microsoft | ~$165 — **CONDITIONAL** | S1–S2 | **Not by default.** |
| 9 | dbt Fundamentals | dbt Labs | Free | S2 | Cost-optimization marts. |
| 10 | ⚡ Dataframe Engine Boundary — Polars-first pipelines | Polars User Guide (roadmap S2 row 6.5) | Free | S2 | Metric computation. ⚠️ **Honest framing: a consistency decision, not a performance one** — the dataset is small; the point is one engine policy across every repo. |
| 11 | Astronomer Academy — Airflow 101 + DAG Authoring | Astronomer | Free | S2 | Scheduled content/pricing ingestion. |
| 12 | Apache Kafka 101 | Confluent Developer | Free | S2 | API/content ingestion patterns. |
| 13 | Terraform Fundamentals | HashiCorp Developer | Free | S2 | Infrastructure. |
| 14 | 🎖️ **DP-700** Fabric Data Engineer | Microsoft | **$165** ✅ | S2 | Shared committed cert; carries over. |
| 15 | 🎖️ **AWS DEA-C01** Data Engineer Associate | AWS | **$150** ✅ | S2 | Shared committed cert; carries over. |
| 16 | AI Agents in LangGraph | DeepLearning.AI | Free | S3 | Opens S3 — agentic rotation with mandatory HITL. |
| 17 | LangChain Academy (LangGraph + LangSmith) | LangChain | Free | S3 | Tracing. |
| 18 | Automated Testing for LLMOps | DeepLearning.AI | Free | S3 | LLM-as-judge eval gate. |
| 19 | 🎖️ **Anthropic CCA-F** | Anthropic · Pearson VUE | **~$125** ✅ ⚠️ | S3 | Optional; shared — not required for a backlog project. |
...
> ⛔ **The IBM AI-Native DE PC is deliberately absent from this table** — no corpus, no vector layer and no training data on a backlog consumer app, so all three gap-filling courses have nothing to bite on. Recorded as a decision, not an omission.
